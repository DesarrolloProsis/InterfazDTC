...
methods: {
  modalAgregarComp: function (){
     ...
     if(this.Editar){
       this.TraercomponentesDTCEditar();
     }else{
  ...
  async TraercomponentesDTCEditar(){
    try {
      const data = await fetch(`${API}/AnexoDTC/ComponentesRequest/${this.plazareferencia}/${this.dtcreference}`)
      const objeto = await data.json();
      this.arraycomponentes = objeto.result;
      console.log(this.arraycomponentes);
    } catch (error) {
