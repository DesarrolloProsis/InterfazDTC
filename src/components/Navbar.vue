    ...
    <div v-if="$route != undefined && $route.name != 'login'" class="relative mr-3">      
      <transition name="fade">
        <div v-if="navbarOpen" class="absolute top-0 mt-1 right-0 w-auto  flex font-titulo">     
          <div class="bg-white  border-r-0 border border-blue-800 p-3 pr-1 pt-0 w-52 rounded-lg" :class="{'bg-white border-r border-blue-800 p-3 pr-1 pt-0 w-52 rounded-lg':typeUser != 7}">                  
          ...
          <div class="bg-white border border-blue-800 p-3 pr-1 pt-0 w-52 rounded-lg sm:hidden" :class="{'h-18 border-l-0':typeUser != 7}">                  
            <div v-for="(item, key) in listaMenuDer" :key="key" class="rounded-2xl p-1 pl-0 mt-1" :class="{'hidden':typeUser != 7}">
              <p class="font-bold">{{ item.texto }}</p>                  
                <div v-for="(itemsub, keySub) in item.subMenu" :key="keySub">
