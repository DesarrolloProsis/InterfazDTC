<template>
    <div class="font-titulo">
        <div v-if="forma == ''">
            ...
            <p class="w-48 input ml-20 sm:ml-1 md:mx-auto" :class="{'sm:w-32':vista=='Actividades', 'sm:w-62':vista=='ConcentradoDTC', 'sm:w-62':vista=='GMMEP', 'sm:w-full sm:ml-0':vista=='Inventario', 'sm:w-full sm:-ml-0':vista=='Encargados'}">
            <select v-model="plazaSelect" @change="actualizar_plaza" :disabled="boolCambiarPlaza" class="w-48 is_valid" :class="{'sm:w-32 sm:text-xs':vista=='Actividades', 'sm:w-63':vista=='Calendario', 'sm:w-62':vista=='ConcentradoDTC', 'sm:w-62':vista=='GMMEP', 'sm:w-full sm:ml-0':vista=='Inventario','sm:w-full sm:-ml-2':vista=='Encargados'}" type="text" name="TipoDescripcion">
